...
public class ListenActivity extends Activity implements OnClickListener,
	...
	public static final List<Auth> authList = Collections
			...
			if (type != null && type.equals(BUNDLE_TYPE_WIFICHANGE)) {
				...
				Toast t = Toast
						.makeText(getApplicationContext(),
								getString(R.string.toast_wifi_lost),
								Toast.LENGTH_SHORT);
				t.show();
				stopListening();
				stopSpoofing();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (currentapiVersion >= Build.VERSION_CODES.KITKAT) {
			...
			int actionBarColor = getResources().getColor(
					R.color.droidsteal_theme_color);
			...
			// barTint.setNavigationBarTintEnabled(true);
		}
	...
	protected void onStart() {
		...
		// showUpdate(); // Disabled -- currently broken/unreliable and annoying
		// for users

		// only show the disclaimer on the first application run
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		// Checks new orientation on change
		// This is entirely unnecessary but I think most will find using the app
		// in landscape laughable at best
		if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
			Toast.makeText(this, "lol landscape", Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void showUpdate() {
		...
		long dif = now - last;
		try {
			if (dif > (1 * 24 * 60 * 60 * 1000)) { // show once per day. (I
													// think)
				// Update Checker
				String VERSION_URL = "https://raw.github.com/Zbob750/DroidSteal/master/update_version.php";
				String REMOTE_APK_URL = " "; // I need to figure out how I want
												// to do this still. :(
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		menu.add(Menu.NONE, ID_NORMAL, Menu.NONE,
				getString(R.string.menu_open_normal));
		if (actElem.getMobileUrl() != null) {
			menu.add(Menu.NONE, ID_MOBILE, Menu.NONE,
					getString(R.string.menu_open_mobile));
		}
		menu.add(Menu.NONE, ID_REMOVEFROMLIST, Menu.NONE,
				getString(R.string.menu_remove_from_list));
		menu.add(Menu.NONE, ID_BLACKLIST, Menu.NONE,
				getString(R.string.menu_black_list));
		menu.add(Menu.NONE, ID_EXPORT, Menu.NONE,
				getString(R.string.menu_export));

		if (actElem.isSaved()) {
			menu.add(Menu.NONE, ID_DELETE, Menu.NONE,
					getString(R.string.menu_delete));
		} else {
			menu.add(Menu.NONE, ID_SAVE, Menu.NONE,
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		MenuItem menu0 = menu.add(0, MENU_CLEAR_SESSIONLIST_ID, 0,
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			...
			builder.setMessage(R.string.popup_exit)
					...
					.setNegativeButton(R.string.button_abort,
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int id) {
									dialog.cancel();
	...
	private boolean isListening() {
		...
		for (RunningServiceInfo service : manager
				.getRunningServices(Integer.MAX_VALUE)) {
			if (DroidSheepService.class.getName().equals(
					service.service.getClassName())) {
				return true;
			}
		}
	...
	private void click(int id, boolean mobilePage) {
		if (authList.isEmpty()) {
	...
	void refreshStatus() {
		...
		if (listening && !spoofing) {
			actionbar.setSubtitle("RUNNING");
			setProgressBarIndeterminateVisibility(true);
		} else if (listening) {
			actionbar.setSubtitle("SPOOFING");
			setProgressBarIndeterminateVisibility(true);
		} else if (spoofing) {
			actionbar.setSubtitle("SPOOFING ONLY"); // this shouldn't occur
													// usually
			setProgressBarIndeterminateVisibility(true);
		} else {
			actionbar.setSubtitle("IDLE");
	...
	private void updateNetworkSettings() {
		...
		if (wifiInfo == null) {
			...
			tnetworkName.setText(getString(R.string.label_networkname_pref)
					+ networkName.toUpperCase(Locale.getDefault()));
		} else {
			String networkName = wifiInfo.getSSID() != null ? " "
					+ wifiInfo.getSSID() : "";
			tnetworkName.setText(getString(R.string.label_networkname_pref)
					+ networkName.toUpperCase(Locale.getDefault()));
		}
	...
	@SuppressLint("NewApi")
	// if statement should handle these API issues
	private void notifyUser(boolean persistent, Auth auth) {
		...
		String notificationText = auth != null ? auth.getUrl()
		...
		if (currentapiVersion >= Build.VERSION_CODES.JELLY_BEAN) {
			notification = new Notification.Builder(context)
					...
					.setContentIntent(contentIntent).build(); // Above if
																// statement
																// should handle
																// API issues
			mNotificationManager.notify(NOTIFICATION_ID, notification);
		} else {
			Notification notificationICS = new Notification(icon,
			...
			if (persistent) {
				...
				notificationICS.setLatestEventInfo(context,
